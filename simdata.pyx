...
# for entire script: python -m cProfile myscript.py
...
# 10 Aug 2023:
# Trying out recasting the Decimals as floats so I can use a different optimizer that is hopefully faster
# Need to figure out which param values to use, because using all of them creates almost 300 sets....
# I moved the floats and BFGS to a different script

# ml_syndata_fast.py (09 Aug 2023):
# removed a bunch of comments, but for now they are all still in ml_syndata_#print.py
...
dtp = np.float64
itp = np.int32
ctypedef np.float64_t dtp_t
ctypedef np.int32_t itp_t
...
rng = np.random.default_rng(seed=61389)

########################################################################################################
############################### LOAD VALUES ############################################################
# region ###############################################################################################

# # Load some real values for storm probabilities and nest initiation dates
...
# This function makes storms - must be called AFTER the param set is specified
...
# cdef np.ndarray in1d_sorted(np.ndarray A, np.ndarray B): # possible days observer could see nest is intersection of observable and survey days
    # print("search for B:", B, "in A:", A) 
    
    idx = np.searchsorted(B, A)
    # cdef np.ndarray idx = np.searchsorted(B, A)
    idx[idx==len(B)] = 0
    return A[B[idx] == A]
...
def mk_obs(params, storm_days, survey_days, dates, initprob, start): 
# cdef mk_obs(
#         int[:] params, int[:] storm_days, int[:] survey_days, int[:] dates, 
#         double[:]initprob, int start
#         ): 
    # cdef int num_nests      = params[0].astype(int)    # make these local variables
    num_nests      = params[0].astype(int)    # make these local variables
    # cdef double probSurv       = params[1] 
    probSurv       = params[1] 
    # cdef double SprobSurv      = params[2]   
    SprobSurv      = params[2]   
    # cdef int obs_int        = params[6].astype(int)
    obs_int        = params[6].astype(int)
    #hatchTime      = params[5].astype(int)     # why does this one not work? no actual issue
    # cdef int hatch_Time     = params[5].astype(int)     # why does this one not work?
    hatch_Time     = params[5].astype(int)     # why does this one not work?
    # cdef int breedingDays   = params[9].astype(int)
    breedingDays   = params[9].astype(int)
    # cdef double probMortFlood  = params[7]
    probMortFlood  = params[7]
    # cdef double discProb       = params[10]
    double discProb       = params[10]

    # check the params that were loaded:
    print(
        "number of nests:", num_nests, "| days til hatch:", hatch_Time, 
        "| length of season:", breedingDays
        )
    
    print(
        "probability of: survival-", probSurv,"; storm survival-", SprobSurv,
        "; storm/flood mortality-", probMortFlood, "; discovery-", discProb
        )
    ...
    print("probability vector:", probVec)
    ...
    nestData       = np.zeros(shape=(num_nests, nCol)) # empty array to fill w/ nest data for each replicate
    ...
    # ----------------------------------------------------------------------------------------------------
    # Column 2 = nest initiation date, drawn from distribution of real nest initiation dates in SWLA
        ...
        if len(alive_days) > 0:
            ...
            if True in discover: # if at least one value is true, nest is discovered
                ...
                if False in alive:  # if there is at least one "False" in "alive" then the nest did not hatch
                    ...
                    j = obsDays[-1].astype(int) # for failed nests, j = last day nest was observed as active
                    nestData[n,5] = j
                ...
                # hatch is more difficult to detect than failure:   
                    ...
                    if np.max(obsDays) == hatch: # if hatch occurs on last obs day, don't add an extra obs day
                        # j = np.max(obsDays)
                        j = np.max(obsDays).astype(int)      # j and k are both hatch day because the nest is still "alive"
                        nestData[n,5] = j
                        # k = np.max(obsDays)
                        k = np.max(obsDays).astype(int)
                        nestData[n,6] = k                  
                    else:
...
#@profile
def like(
        a_s, a_mp, a_mf, a_ss, a_mfs, a_mps, nestData, 
        obs_int, storm_days, survey_days
        ):
    ...
    # FOR EACH NEST -------------------------------------------------------------------------------------
        ...
        print('nest:',nest[0],'| first found:',disc,'| last observed',endObs,'| fate', fate)
        ...
        if len(obsDays) > 1:
            ...
            op = list(itertools.pairwise(obsDays))
            print(op)
            obsPairs = np.array(op)
            #    dtype is pair of ints
            #    but getting error "setting array element with a sequence"
            # obsPairs = np.fromiter(itertools.pairwise(obsDays), dtype=np.dtype(("i,i"))) # do elements of numpy arrays have to be floats?
            # print("date pairs in observation period:", obsPairs) 
            # obsPairs = np.array(obsPairs)
            # obsPairs = np.fromiter(itertools.pairwise(obsDays), dtype=np.dtype(["i,i"])) # do elements of numpy arrays have to be floats?
            #    Field elements must be 2- or 3-tuples, got ''i,i'' 
            #     can't put them in [] I guess
            # obsPairs = np.array(itertools.pairwise(obsDays), dtype=np.dtype(["i,i"])) # do elements of numpy arrays have to be floats?
            print("date pairs in observation period:", obsPairs) 
        else:
            obsPairs = obsDays
        ...
        #    so what was happening when this was python? is this why the output is weird now?
        ...
        # FOR EACH OBSERVATION OF THIS NEST ---------------------------------------------------------------------
            ...
            # but if we were starting at 0 instead of 1, how did that work?
            # i-1 for zero makes the index -1
            print("int element:", intElt)
...
def like_smd(x, ndata, obs, storm, survey):
    ...
    #
    #warning1 = False
    print("transformed vals for optimizer:", s2, mp2, mf2, ss2, mps2, mfs2)
    # ret = like(s2, mp2, mf2, ss2, mps2, mfs2, data)
    ret = like(s2, mp2, mf2, ss2, mps2, mfs2, ndata, obs, storm, survey)
...
# #hatchTime      = [16, 20, 28 ]    # length of incubation - based on real species
# hatchTime      = [19]
...
# As a general rule for optimizing with cython, try to avoid python function calls if possible, 
# i.e. replace pythonic sum with numpy sum or explicit for loop, replace bit number table with 
# numpy array, and try to declare types for all variables.

# You can also precalculate most of the variables used in distance calculation and use a separate 
# array to hold minimum distances for each block. I quickly put something together and got the 
# code below. (from stackexchange)
...
# nrows     = len(paramsList)*nreps*nruns  # number param combos * number reps * number runs
# values    = np.zeros((nrows, num_out)  ) # MLE values for the entire thing
# index     = 0                            # increments for each param set to allow saving MLE vals in chunks
...
# likefile   = Path.home() / '/mnt/c/Users/sarah/Documents/nest_models/py_output' / dir_name / ("ml_values_" + todaysdate)
...
# cpdef run_params(paramsArray, storm_days, vals, dir_name):
cpdef run_params(params, storm_days, vals, dir_name, dates, initprob):
    ...
    # every time the python script calls this function, all these things are
    # made: unpacked params; storage for ll vals and nest data; 
    #       start, survey days; index2 resets; storm days loaded
    ...
    SprobSurv  = params[2]
    stormDur   = params[3].astype(int)
    stormFreq  = params[4].astype(int)
    hatchTime  = params[5].astype(int)
    obs_int    = params[6].astype(int)
    probMortFlood = params[7]
    SprobMortFlood = params[8]
    breedingDays   = params[9].astype(int)
    discProb       = params[10]
    ...
    # storm_days  = stormGen(stormDur, stormFreq)  
    ...
    for r in range(nreps):  # for each data replicate that uses this set of params
        ...
        #rng = np.random.default_rng()        # call random number generator
        # repID       = f'{repID}{idx}'
        like_val    = np.array(num_out) # will be rewritten for each loop 
        repID       = f'{repID}{index2}'
        try:
            nestData    = mk_obs(params,storm_days, survey_days, dates, initprob, start)              # make nest data
        except IndexError:
       ...
       # overrides the seed, so only the first nest data is reproducible
        ...
        try:
            ...
            ans  = optimize.minimize(
                    like_smd, z, 
                    args=(nestData, obs_int, storm_days, survey_days), 
                    method='Nelder-Mead'
                    )
...
#     # change filepath for running script in WSL:
#     filepath     = Path.home() / '/mnt/c/Users/sarah/Documents/nest_models/py_output' / dir_name / ('ml_val_' + dir_name + '.csv')
