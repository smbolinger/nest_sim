...
# from simdata2 import run_params
from simdata import run_params
...
paramsList      = list(product(
        ...
        obs_int, probMortFlood, SprobMortFlood, breedingDays, discProb
        ))
...
vals = np.array([nreps, nruns, num_out])
...
with open(likefile, "wb") as f: 
    ...
    for i in range(0, len(paramsList)): # for each set of params
        ...
        par = paramsArray[i]
        frq = par[4].astype(int)
        dur = par[3].astype(int)

        storm_days = stormGen(frq, dur)
        # main_code = """
        # likeVal = run_params(par, storm_days, vals, dir_name, dates, initprob)
        # """
        likeVal = run_params(par, storm_days, vals, dir_name, dates, initprob)
